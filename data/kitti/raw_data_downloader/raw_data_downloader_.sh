...
helpFunction() {
  echo ""
  echo "Usage: $0 [-d space-seperated-list] [-p prefix]"
  echo -e "\t-d --days the days you want to download(space-seperated list)(default is $days)"
  echo -e "\t-p --prefix the address prefix to save files(default is empty string)"
  echo -e "\t-l --limit limit the number of files(default is -1 means no limit)"
  echo -e "\t --dir the folder to extract data"
  exit 1
}
...
for i in "${files[@]}"; do
  echo -e "$i"
  if [[ ! "${days[*]}" =~ ${i:8:2} ]]; then continue; fi
  if [ "${i:(-3)}" != "zip" ]; then
  ...
  if [ -z "${limit}" ]; then
    echo "limit size reached exiting..."
    break
  fi
  limit="$((limit - 1))"
#  if [ $limit -le 0 ]; then
